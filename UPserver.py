            ...
            if len(player_connections) >= MIN_PLAYERS and not game_in_progress:
                with countdown_timer_lock:
                    ...
                    if not countdown_timer_running:
                        ...
                        # Timer already running, just inform the new player
                        wfile.write(f"[INFO] Game is already counting down and will start soon.\n\n")
                        wfile.write(f"[INFO] Currently {len(player_connections)} player(s) connected. Maximum is {MAX_PLAYERS}.\n\n")
                        wfile.flush()
                        
                        # Notify other players that someone joined
                        for c, _, _, wf, _ in player_connections:
                            if c != conn:  # Don't send to the new player
                                try:
                                    wf.write(f"[INFO] Player {player_num} has joined. ({len(player_connections)}/{MAX_PLAYERS} players)\n\n")
                                    wf.flush()
                                except:
                                    pass
